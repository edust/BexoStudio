...
- 范围：
  ...
  - `scripts/work/2026-04-30-startup-silent-launch-fix/`
- 方案：
  - Windows 下直接写入带引号 exe 路径的 `HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run` 注册值，并携带 `--autostart`。
  - 应用启动时按保存的 `launchAtLogin` 偏好重新同步系统 autostart 状态。
  - 主窗口默认隐藏，由 Rust setup 根据 `--autostart + startSilently` 决定隐藏或显示，避免静默启动闪窗。
- 验证：
  - `cargo check --manifest-path "src-tauri/Cargo.toml"`
  - `npm run web:build`
  - `cargo test --manifest-path "src-tauri/Cargo.toml" --lib --no-run`
  - 手工验证注册表 Run 值、正常启动显示、`--autostart` 静默隐藏。
- 状态：代码修复与自动验证已完成；测试运行阶段仍受仓库已知 `STATUS_ENTRYPOINT_NOT_FOUND` 影响。

## 2026-04-21 打开终端路径前缀修复
- 目标：修复工作区打开终端时 PowerShell 显示 `Microsoft.PowerShell.Core\FileSystem::\\?\...` 的路径问题。
- 范围：
  - `src-tauri/src/services/workspace_service.rs`
  - 根级 planning 文件
  - `scripts/work/2026-04-21-terminal-verbatim-path-fix/`
- 验证：
  - `cargo check --manifest-path "src-tauri/Cargo.toml"`
  - 手工验证工作区终端和资源子目录终端 prompt 不再显示 `\\?\`
- 状态：已完成代码修复与编译验证（2026-04-21）
  - 内部 canonical path 校验保留
  - 传给 Windows Terminal 前已去掉 Windows verbatim prefix
  - `cargo check --manifest-path "src-tauri/Cargo.toml"` 已通过

## 2026-04-20 工作区右键置顶菜单
- 目标：给左侧工作区卡片新增右键菜单，并支持 `置顶 / 取消置顶` 持久化能力。
- 范围：
  ...
  - `src-tauri/src/services/preferences_service.rs`
  - 根级 planning 文件
  - `scripts/work/2026-04-20-workspace-pin-context-menu/`
- 验证：
  - `npm run web:build`
  - `cargo check --manifest-path "src-tauri/Cargo.toml"`
  - 手工验证右键菜单与置顶状态持久化
- 状态：已完成代码修复与构建验证（2026-04-20）
  - 已新增 `workspace.pinnedWorkspaceIds / pinned_workspace_ids` 前后端偏好字段
  - 左侧工作区卡片已支持右键菜单 `置顶 / 取消置顶`
  - 点击 `置顶` 会把目标工作区提到顶部并持久化
  - `cargo check --manifest-path "src-tauri/Cargo.toml"` 与 `npm run web:build` 已通过

## 2026-04-20 工作区侧栏拖拽排序修复
- 目标：修复左侧工作区侧栏拖拽排序时出现的“下面部分消失”和“顺序异常”问题。
- 范围：
  - `src/components/shell/section-sidebar.tsx`
  - 根级 planning 文件
  - `scripts/work/2026-04-20-workspace-sidebar-reorder-fix/`
- 验证：
  - `npm run web:build`
  - 手工验证滚动列表也可拖、拖拽后顺序稳定
- 状态：已完成代码修复与构建验证（2026-04-20）
  - `Reorder` 已改用稳定的 `workspace.id`，避免 query 回流时对象引用变化打坏拖拽预览
  - 滚动容器已改为 `motion.div layoutScroll`，恢复长列表拖拽
  - 拖拽项已补 `relative`，保证拖动中的层级效果
  - `npm run web:build` 已通过

## 2026-04-20 终端命令特殊字符保存修复
- 目标：修复 Home 页“新增终端命令”里输入包含 Windows 路径、反斜杠、引号等特殊字符时，保存后被改写的问题。
- 范围：
  - `src/lib/terminal-command.ts`
  - `src/pages/home-page.tsx`
  - `src-tauri/src/services/workspace_service.rs`
  - 根级 planning 文件
  - `scripts/work/2026-04-20-terminal-command-special-char-fix/`
- 验证：
  - 自动化测试覆盖 `cd "C:\\abcd"` 等路径场景
  - `npm run web:build`
  - `cargo test --manifest-path "src-tauri/Cargo.toml" workspace_service -- --nocapture`
- 状态：已完成代码修复与构建验证（2026-04-20）
  - 已修复前端命令解析与展示重建，Windows 路径中的 `:`、`\`、引号不再在保存前被吞掉
  - 已修复 Rust 侧 PowerShell 命令重组，改为 call operator + 单引号字面量 token
  - `npm run web:build`、`cargo check --manifest-path "src-tauri/Cargo.toml"` 已通过
  - Rust 单元测试已编译并注入，但当前环境执行阶段卡在 `0xc0000139 (STATUS_ENTRYPOINT_NOT_FOUND)`

## 2026-04-20 截图选区外单击误清空并冻结修复
- 目标：修复截图态下切回 `select` 后单击选区外区域，会误清空标注并最终冻结的问题。
- 范围：
  - `src/pages/screenshot-overlay-page.tsx`
  - 根级 planning 文件
  - `scripts/work/2026-04-20-screenshot-outside-click-freeze/`
- 验证：
  - `npm run web:build`
  - 手工验证外部单击不再清空对象，外部拖框仍可重建新选区
- 状态：已完成代码与构建验证，待用户本机手工回归（2026-04-20）

## 2026-04-20 截图底部工具栏单行 Icon Rail
- 目标：把 screenshot overlay 底部 toolbar 改成单行 icon-only 工具带，文字只在 hover tooltip 里显示。
- 范围：
  - `src/pages/screenshot-overlay-page.tsx`
  - 根级 planning 文件
  - `scripts/work/2026-04-20-screenshot-toolbar-icon-rail/`
- 验证：
  - `npm run web:build`
  - 手工验证单行 icon、tooltip、横向滚动、左右边缘安全定位
- 状态：已完成代码与构建验证，待用户本机手工回归（2026-04-20）

## 2026-04-20 截图工具栏移除颜色按钮与颜色切换
- 目标：删掉 screenshot overlay 的颜色按钮和颜色切换能力，让标注先退回固定单色。
- 范围：
  - `src/pages/screenshot-overlay-page.tsx`
  - 根级 planning 文件
  - `scripts/work/2026-04-20-screenshot-remove-color-controls/`
- 验证：
  - `npm run web:build`
  - 手工验证工具栏无颜色按钮、标注仍可创建、默认统一单色
- 状态：已完成代码与构建验证，待用户本机手工回归（2026-04-20）

## 2026-04-19 截图选框视觉与拖动稳定性修复
- 目标：修复截图选框移动时边线缺段，并恢复选区外灰淡透明遮罩。
- 范围：
  - `src-tauri/src/services/native_interaction_backend_windows.rs`
  - 根级 planning 文件
  - `scripts/work/2026-04-19-screenshot-selection-visual-hardening/`
- 验证：
  - `cargo check --manifest-path "src-tauri/Cargo.toml"`
  - 手工验证遮罩、拖动/缩放边线完整性、toolbar exclusion 透传
- 状态：已完成代码与编译验证，待用户本机手工回归（2026-04-19）

## 2026-04-19 截图跨家族混选方向键微调
- 目标：补齐 screenshot overlay 中 mixed selection 的方向键微调，让跨家族混选与现有整组拖动/复制/删除/层级能力对齐。
- 范围：
  - `src/pages/screenshot-overlay-page.tsx`
  - 根级 planning 文件
  - `scripts/work/2026-04-19-screenshot-mixed-selection-nudge/`
- 验证：
  - `npm run web:build`
  - 手工验证跨家族混选 `Arrow* / Shift+Arrow*`、复制/删除/层级、选中态保持
- 状态：已完成代码与构建验证，待用户本机手工回归（2026-04-19）
...
- 目标：修复截图 overlay 在已有选区时，左键点击选区外区域立即冻结/假死的问题。
- 范围：
  - `src/pages/screenshot-overlay-page.tsx`
  - `src-tauri/src/services/native_interaction_backend_windows.rs`
  - `scripts/work/2026-04-20-screenshot-outside-click-immediate-freeze/*`
- 方案：
  - 将“选区外单击”和“从选区外拖出新框”拆成两条状态路径。
  - `pointerdown` 落在选区外时不再立刻进入 `dragStart`，而是先记录待判定起点。
  - 只有 `pointermove` 超过阈值后，才真正切换到重画选区态；纯点击只清对象选中态，不改当前选区。
  - 保持现有“有效新框才 `resetAnnotations()`”语义，不回退上一轮防崩修复。
  - 若最新复现继续绕过前端分支，则继续下沉到 Rust Native Interaction backend，保证 Win32 消息线程上的命中测试/鼠标消息不会阻塞等待共享状态锁。
- 验证：
  - `npm run web:build`
  ...
  - 手工验证：
    ...
    - 外部拖拽仍可重画新选区
    - 添加对象后外部点击不冻结、不丢对象
- 状态：前端外部点击分支与 Rust Native Interaction 消息线程防阻塞补丁均已完成，`npm run web:build` 与 `cargo check` 已通过，待用户手工回归
  - 2026-04-20 补充：继续收口后确认消息线程后半段仍会在 `present_interaction_surface()` 内走阻塞锁。
  - 已改为 `wm_lbuttondown / wm_mousemove / wm_lbuttonup` 使用非阻塞 present；拿不到共享锁时直接跳过该帧重绘，优先保证 Win32 消息泵可返回。
  - 最新验证：`cargo check --manifest-path "src-tauri/Cargo.toml"` 通过。
  - 2026-04-20 再补充：最新复现表明“选区外点击”在日志上没有再落进锁竞争分支，而是更像 `HTTRANSPARENT` 透传后的下层窗口挂起。
  - 已改为在 `selection` 模式且已有选区时吞掉选区外点击，不再把这类点击继续透传给 overlay/WebView；toolbar/text editor exclusion rect 保持不变。
  - 2026-04-20 再补充：用户回归确认外部点击不再冻结，但 `Esc` 无法退出截图态。
  - 已补会话级 `Escape` Windows hook：Rust 捕获 `Esc` 后发 `screenshot://escape-pressed` 事件回前端，复用 overlay 既有的分级 `Esc` 处理逻辑，不再依赖 WebView 焦点。

## 2026-04-20 移除截图工具热键配置
- 目标：彻底移除 Settings > Hotkeys 中“截图工具热键（截图界面内）”能力，并收口相关类型、偏好存储与 overlay 读取链路。
- 范围：
  - `src/pages/settings-page.tsx`
  - `src/pages/screenshot-overlay-page.tsx`
  - `src/lib/app-preferences.ts`
  ...
  - `src-tauri/src/domain/preferences.rs`
  - `src-tauri/src/services/preferences_service.rs`
  - `scripts/work/2026-04-20-remove-screenshot-tool-hotkeys/*`
- 方案：
  - 删除设置页中截图工具热键配置区块，以及对应的草稿状态、保存/恢复默认、去重校验和辅助函数。
  - 删除 `AppPreferences.hotkey.screenshotTools` 前后端类型、默认值与持久化字段。
  - overlay 不再从偏好加载截图工具热键，改回使用固定工具热键映射。
  - 保留截图界面内部固定工具切换能力，不再暴露给设置页配置。
- 验证：
  - `npm run web:build`
  - `cargo check --manifest-path "D:\Desktop\rust\BexoStudio\src-tauri\Cargo.toml"`
  - 手工验证：
    - Settings > Hotkeys 红框区块完全消失
    - 打开截图界面后 `1~5/6~0/n` 工具切换仍正常
    - 已有历史偏好数据时设置页与截图页均不报错
- 状态：已完成代码清理与编译验证（2026-04-20）
  - 已删除设置页中的截图工具热键卡片及其保存/恢复默认/去重辅助逻辑
  - 已删除前后端 `hotkey.screenshotTools / screenshot_tools` 类型、默认值与偏好校验
  - overlay 已改回固定工具热键映射，不再读取偏好
  - `cargo check --manifest-path "src-tauri/Cargo.toml"` 与 `npm run web:build` 已通过

## 2026-04-21 Terminal Command Shell Quoting Fix
- 目标：修复 Home 终端命令保存后丢失路径引号，以及 Windows Terminal/PowerShell 下运行 `cd ... && ...` 报错的问题。
- 范围：
  - Home 终端命令编辑与保存链路
  - LaunchTask `terminal_command` 校验/持久化链路
  - Windows Terminal adapter 命令组装链路
  - `scripts/work/2026-04-21-terminal-command-shell-quoting/*`
- 方案：
  - 前端保存时保留用户输入的原始单行命令，不做破坏性 token 化/反序列化。
  - Rust 启动层明确 Windows shell 语义，避免 cmd 风格 `&&` 被旧 PowerShell 当作 `Set-Location` 参数解析。
  - 不改 DB schema，不新增自动重试。
- 验证：
  - `npm run web:build`
  - `cargo check --manifest-path "src-tauri/Cargo.toml"`
  - 手工验证保存后重新编辑仍保留 `cd "...\\backend" && go run ./cmd` 的引号，并能从 Windows Terminal 启动。
- 状态：已完成代码修复与自动验证。`cargo test --lib shell_command_line` 运行阶段仍受仓库已知 `STATUS_ENTRYPOINT_NOT_FOUND` 影响，但 `cargo test --lib --no-run` 已通过。

## 2026-04-30 Codex History View
- 目标：在 Workbench 工作区卡片上新增只读 Codex 历史入口，点击后按工作区打开独立历史窗口。
- 范围：
  - `src-tauri/src/domain/codex_history.rs`
  - `src-tauri/src/services/codex_history_service.rs`
  - `src-tauri/src/commands/codex_history.rs`
  - `src-tauri/src/app/mod.rs`
  - `src-tauri/capabilities/default.json`
  - `src/types/backend.ts`
  - `src/lib/command-client.ts`
  - `src/components/shell/section-sidebar.tsx`
  - `src/pages/codex-history-window-page.tsx`
  - `src/app/app.tsx`
  - `scripts/work/2026-04-30-codex-history-view/*`
- 方案：
  - Rust 侧只读扫描 Codex `sessions/**/*.jsonl`，解析 `session_meta.cwd` 并过滤到当前工作区路径内。
  - 同时纳入默认 `CODEX_HOME` / `~/.codex` 和已保存 Codex Profile 的 `codex_home`。
  - session 列表读取文件头尾，不全量加载；消息详情从文件尾部反向分页，初始返回最新一页。
  - 前端在工作区卡片复制路径按钮前新增历史按钮，打开 `?window=codex-history&workspaceId=<id>` 独立窗口。
  - 历史窗口左侧 session 列表支持搜索、刷新和虚拟滚动，右侧消息流向上滚动加载更早消息。
- 验证：
  - `cargo fmt --manifest-path "D:\Desktop\rust\BexoStudio\src-tauri\Cargo.toml"`
  - `cargo check --manifest-path "D:\Desktop\rust\BexoStudio\src-tauri\Cargo.toml"`
  - `cargo test --manifest-path "D:\Desktop\rust\BexoStudio\src-tauri\Cargo.toml" --lib --no-run`
  - `npm run web:build`
- 状态：代码实现与自动验证已完成；真实桌面窗口打开、多工作区并行和滚动加载仍需本机手工回归。

## 2026-04-30 Global Codex History Page
- 目标：补齐左侧主导航 Session/History 总入口，提供全局只读 Codex sessions 查看页。
- 范围：
  - `src-tauri/src/domain/codex_history.rs`
  - `src-tauri/src/services/codex_history_service.rs`
  - `src-tauri/src/commands/codex_history.rs`
  ...
  - `src/components/shell/primary-rail.tsx`
  - `src/routes/app-router.tsx`
  - `src/types/backend.ts`
  - `src/lib/command-client.ts`
  - `src/pages/history-page.tsx`
  - `scripts/work/2026-04-30-global-codex-history-page/*`
- 方案：
  - 新增 `list_all_codex_history_sessions()` 命令，读取全部已知 Codex sessions 根。
  - `get_codex_history_messages` 的 `workspaceId` 改为可选：工作区窗口仍强校验，主页面只校验 source 在 sessions 根内。
  - 左侧 primary rail 新增 `Session / History` 图标入口，路由为 `/history`。
  - `/history` 页面显示全局 session 列表，支持工作区路径筛选、搜索、刷新和消息反向分页。
- 验证：
  - `cargo fmt --manifest-path "D:\Desktop\rust\BexoStudio\src-tauri\Cargo.toml"`
  - `cargo check --manifest-path "D:\Desktop\rust\BexoStudio\src-tauri\Cargo.toml"`
  - `cargo test --manifest-path "D:\Desktop\rust\BexoStudio\src-tauri\Cargo.toml" --lib --no-run`
  - `npm run web:build`
- 状态：实现中，已通过初次 `cargo check` 与 `npm run web:build`，待最终验证和手工回归。
  - 最终自动验证已完成：`cargo fmt`、`cargo check`、`cargo test --lib --no-run`、`npm run web:build` 均通过。
  - 消息阅读器已抽出共享 `CodexHistoryMessageBlock`，用户消息和 Codex 最终回复默认展开。
  - 技术项显示规则已抽出共享 helper，工具调用/输出/推理/联网搜索以及 AGENTS/context 规则块默认隐藏；勾选 `显示技术项` 后才显示，并继续默认折叠。
  - 折叠与隐藏 refinement 已通过 `npm run web:build`。
  - 待桌面 runtime 手工回归左侧 History 入口、全局 session 列表、工作区筛选和向上分页。

## 2026-05-01 Codex History Font Settings
- 目标：在 Settings > General 增加 Codex 历史对话字体名称和字号设置，默认字号为 12px。
- 范围：
  - `src-tauri/src/domain/preferences.rs`
  - `src-tauri/src/services/preferences_service.rs`
  - `src/types/backend.ts`
  - `src/lib/app-preferences.ts`
  - `src/pages/settings-page.tsx`
  - `src/features/codex-history/*`
  - `src/pages/history-page.tsx`
  - `src/pages/codex-history-window-page.tsx`
  - `scripts/work/2026-05-01-codex-history-font-settings/*`
- 方案：
  - 在现有 `AppPreferences` 中新增 `codexHistory` 偏好组，不新增独立存储。
  - Rust 侧负责默认值、旧配置兼容、非法持久化值修复和更新请求校验。
  - History 页面从偏好查询中读取字体设置，消息内容渲染由共享组件统一应用。
- 验证：
  - `cargo fmt --manifest-path "D:\Desktop\rust\BexoStudio\src-tauri\Cargo.toml"`
  - `cargo check --manifest-path "D:\Desktop\rust\BexoStudio\src-tauri\Cargo.toml"`
  - `npm run web:build`
